    ...
    async def stream_chat(
        ...
        async with self._client.stream(
            ...
            async for line in response.aiter_lines():
                ...
                if "error" in event:
                    raise RuntimeError(
                        f"Groq API error: {event['error'].get('message', event['error'])}"
                    )
                choice = event["choices"][0]
                delta_obj = choice.get("delta", {})
                delta = delta_obj.get("content", "") or ""
                finish_reason = choice.get("finish_reason")
                usage = event.get("x_groq", {}).get("usage", {})

                for frag in delta_obj.get("tool_calls") or []:
                    slot = pending_calls.setdefault(frag["index"], {"id": None, "name": None, "arguments": ""})
                    if frag.get("id"):
                        slot["id"] = frag["id"]
                ...
                if finish_reason == "tool_calls":
                    ...
                    for slot in pending_calls.values():
                        try:
                            ...
                            print(
                                f"Groq: skipping malformed tool call arguments for "
